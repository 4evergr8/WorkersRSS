...
export async function itunes(artistIdOrName, baseUrl) {
    ...
    if (isName) {
        ...
        if (!searchResp.ok) {
            throw new Error(`iTunes Search API 请求失败: ${searchResp.status}`);
        }
    ...
    if (!resp.ok) {
        throw new Error(`iTunes Lookup API 请求失败: ${resp.status}`);
    }

    const data = await resp.json();
    if (!data.results || data.results.length === 0) {
        throw new Error("未找到该歌手的任何数据");
    }
    ...
    const currentRssUrl = baseUrl ? `${baseUrl}?itunes=${artistId}` : "";
    ...
    const feed = new Feed({
        ...
        id: artistLink,
        link: artistLink,
        image: "https://music.apple.com/assets/favicon/favicon-32.png",
        // 新增 channel 级别的 xml 订阅链接字段
        feedLinks: currentRssUrl ? {
            rss: currentRssUrl
        } : undefined,
        updated: now,
    });
...
<div>
    ...
    <p><a href="${itemUrl}">在 Apple Music 中查看全专</a></p>
</div>
`;
